...
from enterprise_computer_use.tools.base import CLIResult, ToolError, ToolResult
...
async def test_view_command():
    ...
    ), patch("enterprise_computer_use.tools.claude.edit.run") as mock_run:
    ...
    with patch("pathlib.Path.exists", return_value=True), patch(
        ...
        with pytest.raises(ToolError, match="Invalid `view_range`"):
            await edit_tool(
                command="view", path="/test/file.txt", view_range=[3, 2]
            )
    ...
    with patch("pathlib.Path.exists", return_value=True), patch(
        ...
        with pytest.raises(
            ToolError, match="view_range` parameter is not allowed"
        ):
...
async def test_create_command():
    ...
    with patch("pathlib.Path.exists", return_value=False):
        with pytest.raises(
            ToolError, match="Parameter `file_text` is required"
        ):
...
async def test_str_replace_command():
    ...
    with patch("pathlib.Path.exists", return_value=True), patch(
        ...
        assert edit_tool._file_history[Path("/test/file.txt")] == [
            "Original content"
        ]
...
async def test_undo_edit_command():
    ...
    with patch("pathlib.Path.exists", return_value=True), patch(
        ...
        assert result.output
        assert (
            "Last edit to /test/file.txt undone successfully" in result.output
        )
