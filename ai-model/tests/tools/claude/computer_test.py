...
from enterprise_computer_use.tools.claude.computer import (
    ComputerTool,
    ScalingSource,
    ToolError,
    ToolResult,
...
async def test_computer_tool_mouse_move(computer_tool):
    with patch.object(
        computer_tool, "shell", new_callable=AsyncMock
    ) as mock_shell:
...
async def test_computer_tool_type(computer_tool):
    with (
        ...
        assert (
            "type --delay 12 -- 'Hello, World!'" in mock_shell.call_args[0][0]
        )
...
async def test_computer_tool_missing_coordinate(computer_tool):
    with pytest.raises(
        ToolError, match="coordinate is required for mouse_move"
    ):
