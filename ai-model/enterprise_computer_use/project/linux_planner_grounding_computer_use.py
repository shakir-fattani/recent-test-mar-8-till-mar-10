...
from enterprise_computer_use.tools.third_party.cambio.anyparser.anyparser import (
    AnyParserTool,
)
...
If the instruction is long, split the tools into multiple simpler steps.
...
@Registry.register(OSType.LINUX, ProjectType.PLANNER_GROUNDING_COMPUTER_USE)
def _():
    """Register the Linux claude computer use project with the registry."""
    return Config(
        tool_collection=ToolCollection(
            BashTool(), SimpleComputerTool(), AnyParserTool()
        ),
