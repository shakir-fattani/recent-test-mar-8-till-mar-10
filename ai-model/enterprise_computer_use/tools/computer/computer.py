...
from enterprise_computer_use.tools.claude.run import run
...
class ComputerTool(BaseTool):
    """
    A tool that allows the agent to interact with the screen, keyboard, and
    mouse.
    """

    name: ClassVar[Literal["computer_tool"]] = "computer_tool"
    width: int
    height: int
    ...
    def description(self) -> str:
        ...
        if self.INCLUDE_CALL_IN_DESCRIPTION:
            return (
                ...
                f"```python\n{inspect.getsource(self.__call__)}```"
            )
        return doc
    ...
    def to_params(self, **kwargs) -> dict[str, Any]:
        ...
        if model is None:
            raise ValueError("Model must be specified (e.g., 'claude')")
            ...
            return {
                ...
                "input_schema": {
                    ...
                    "properties": {
                        ...
                        "text": {
                            "type": "string",
                            "description": "Description of what to do for the action. Text is only required for below actions: key, type",
                        },
                        "coordinate": {
                            "type": "array",
                            "description": "The (x, y) coordinates of the action. Coordinate is only required for below actions: mouse_move, left_click_drag.",
                            "items": {
                                "type": "integer",
                            },
                        },
